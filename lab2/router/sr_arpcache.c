...
void sr_arpreq_send_packets(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    while (cur) {
        ...
        eth_hdr = (struct sr_ethernet_hdr *)cur->buf;
        memcpy(eth_hdr->ether_dhost, );
        sr_send_packets(sr, cur->buf, cur->len, cur->interface);*/
...
void sr_arpreq_handle(struct sr_instance *sr, struct sr_arpreq *req)
{
    if (difftime(time(NULL), req->sent) > 1.0) {
        if (req->times_sent >= 5) {
            ...
            sr_send_icmp_host_unreachable(sr, req);
...
void sr_send_arp_request(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    /* Create ARP header with appropriate request info */
    arpHdr.ar_hrd = htons(arp_hrd_ethernet);
    arpHdr.ar_pro = htons(arp_pro_ip);
    ...
    sr_encap_and_send_pkt(sr, (uint8_t *)&arpHdr, sizeof(struct sr_arp_hdr), req->ip, 0, ethertype_arp);
}

void sr_send_icmp_host_unreachable(struct sr_instance *sr, struct sr_arpreq *req)
{
    ...
    while (packet) {
        /* Send ICMP */
        sr_send_icmp(sr, packet->buf, packet->len, 3, 1);
        packet = packet->next;
    }
}
